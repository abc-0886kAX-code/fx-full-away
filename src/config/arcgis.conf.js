 ...
 * @LastEditTime: 2022-09-14 11:07:16
 */
const { VUE_APP_ARCGIS_SERVER } = process.env;

const setupServiceAddress = (server = '') =>
    `${VUE_APP_ARCGIS_SERVER}arcgis/rest/services/FX/${server}/MapServer`;

export const setupServiceID = (service, id) => `${service}/${id}`;

export const retainServiceBody = url => {
    const [_, body] = url.split(VUE_APP_ARCGIS_SERVER);
    return body;
};
