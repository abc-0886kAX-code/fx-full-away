 ...
 * @LastEditTime: 2022-07-15 13:58:51
 * @Description: trigger="hover" trigger="click"
-->
<template>
    <el-popover popper-class="screen-grid-select-popover" v-model="visable" :disabled="usePopover" :width="width"
        ...
        <div class="screen-grid-popup">
            <template v-for="item in menuRaw">
                <screen-grid-select-node :key="item.id" :active="activeNode" :cell="item" :children="item.children"
                    @toEvent="useSelectEvent">
...
import { isNil, isArray, isBoolean } from "lodash";
...
const transformMenu = (parent) => (item) => {
    ...
    if (isRender && isArray(children) && children.length > 0)
        raw.children = children.map(transformMenu(raw)).filter(item => !isNil(item));
...
export default {
    ...
    setup(props, context) {
        ...
        const menuRaw = computed(() => unref(menuToPower).map(transformMenu()).filter(item => !isNil(item)));
