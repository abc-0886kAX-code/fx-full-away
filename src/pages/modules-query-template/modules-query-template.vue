<!--
 * @FilePath: \3D防汛作战\src\pages\modules-query-template\modules-query-template.vue
 * @Author: zhangyang
 * @Date: 2022-06-29 10:19:36
 * @LastEditors: zhangyang
 * @LastEditTime: 2022-07-08 13:20:58
 * @Description: 
-->
<template>
    <mg-table class="modules-body modules-query-template" v-loading="tableLoading" v-bind="loadStyle"
...
export default {
    ...
    setup() {
        const {
            ...
            setupUiSchema,
            setupRowController,
        } = useTableData("modules-query-template");
        ...
        const executeQuery = async (params = {}) => {
            setupLoading();
            const data = await setupTableData(icmQueryRunTemplate(params));
            return { data };
        };
...
@import "./modules-query-template.scss";
