 ...
 * @LastEditTime: 2022-07-06 10:18:23
 * @Description: 
-->
<template>
    <div class="water-routing">
        <div class="water-routing-brief">
            <briefing :dataStructure="dataStructure"></briefing>
        </div>
        <div class="water-routing-chart">
...
import { getModelResultData } from "@/pages/basin-flood/api/calculus.api";
...
// 格式化日期时间
// const hanlderTimeFormat = (baseTime) => (time) => {
//     return isSameDay(new Date(baseTime), new Date(time))
//         ? format(new Date(time), "HH时")
//         : format(new Date(time), "dd日HH时");
// };
        ...
        xAxis: [
            {
                ...
                axisLabel: {
                    color: "#fff",
                    // formatter: hanlderTimeFormat(data.times),
                },
                data: rainsX,
            },
            {
...
export default {
    ...
    setup(props) {
        ...
        onMounted(async () => {
            const [response] = await getModelResultData(unref(params));
