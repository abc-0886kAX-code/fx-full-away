...
-->
<template>
    <div class="basic-data-cbh">
        <el-tree class="tree-check" :data="treeMenu" :show-checkbox="true" :default-expand-all="true"
            :check-on-click-node="true" :expand-on-click-node="false" node-key="key" @check-change="handlerCheckChange">
        ...
        <template v-for="tmp in treeDataSource">
            <component v-if="tmp.render" :key="tmp.key" :is="tmp.componentName" :node="tmp" :attr="tmp.props">
            </component>
        </template>
    </div>
</template>
