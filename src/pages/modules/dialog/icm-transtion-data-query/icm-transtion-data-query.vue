<!--
 * @FilePath: \3D防汛作战\src\pages\modules\dialog\icm-transtion-data-query\icm-level-data-query.vue
 * @Author: zhangyang
 * @Date: 2022-06-30 16:01:00
 * @LastEditors: zhangyang
 * @LastEditTime: 2022-07-04 10:30:02
 * @Description: 
-->
<template>
    <mg-table class="modules-body icm-transtion-data-query" v-loading="tableLoading" v-bind="loadStyle"
...
import { icmQueryTranstionData } from "@/api/icm/query.api";

import { default as DialogContentMixins } from "@/mixins/dialog-content.mixins";

export default {
    name: "icm-transtion-data-query",
    mixins: [DialogContentMixins],
    components: {},
    props: {},
    setup(props) {
        const { dialogLoadStart, dialogLoadEnd } = inject("dialog");
        const { table, setupTableData, setupTableColumn, setupUiSchema } =
            useTableData("icm-transtion-data-query");
        ...
        const executeQuery = async () => {
            dialogLoadStart();
            const { datacase, columns } = await icmQueryTranstionData(props.info);
...
@import "./icm-transtion-data-query.scss";
